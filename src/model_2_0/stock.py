...
class Stock():
    ...
    def get_n_last_div_per_share(self, n):
        '''
        n: int, or 'all'
        '''

        n = (self.company.dividends.size if n == 'all'
             else min(n, self.company.dividends.size))

        dividend_payout = self.company.dividends.iloc[-n:]
        n_shares = self.company.n_shares
    ...
    def get_adj_closing_prices(self):
        P = np.array(self.prices)
        dividends = self.get_n_last_div_per_share('all')  # pd.Series
        D = np.zeros_like(P)
        for k, v in dividends.iteritems():
            D[k] = v
        return D.cumsum() + P
